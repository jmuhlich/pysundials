...
def PrintHeader(globalstrategy, maxl, maxlrst, fnormtol, scsteptol, linsolver):
	...
	if linsolver == USE_SPGMR:
		...
		print "Linear solver is SPBCG with maxl = %d"%(maxl)
	elif linsolver == USE_SPTFQMR:
		print "Linear solver is SPTFQMR with maxl = %d"%(maxl)
...
def PrintFinalStats(kmem, linsolver):
...
for linsolver in range(3):
	...
	if linsolver == USE_SPGMR:
		...
		print " -------"
		maxl = 15
		kinsol.KINSpbcg(kmem, maxl)
		kinsol.KINSpilsSetPreconditioner(kmem, PrecSetupBD, PrecSolveBD, ctypes.pointer(data))
	elif linsolver == USETFQMR:
		print " --------- "
		print "| SPTFQMR |"
		print " ---------"
		maxl = 25
		kinsol.KINSptfqmr(kmem, maxl)
		kinsol.KINSpilsSetPreconditioner(kmem, PrecSetupBD, PrecSolveBD, ctypes.pointer(data))

	PrintHeader(globalstrategy, maxl, maxlrst, FTOL, STOL, linsolver)
