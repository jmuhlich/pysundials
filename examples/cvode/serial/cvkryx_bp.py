...
def PrintFinalStats(cvode_mem, bpdata):
	...
	(lenrwBP, leniwBP) = cvode.CVBandPrecGetWorkSpace(bpdata)
	nfeBP = cvode.CVBandPrecGetNumRhsEvals(bpdata)
...
for jpre in range(cvode.PREC_LEFT, cvode.PREC_RIGHT+1):
	if jpre == cvode.PREC_RIGHT:
		SetInitialProfiles(u, data.dx, data.dy)
		cvode.CVodeReInit(cvode_mem, f, T0, u, cvode.CV_SS, reltol, abstol)
		cvode.CVSpilsSetPrecType(cvode_mem, cvode.PREC_RIGHT)
		print "\n\n-------------------------------------------------------------------"
	...
	tout = TWOHR
	while iout <= NOUT:
		flag = cvode.CVode(cvode_mem, tout, u, ctypes.byref(t), cvode.CV_NORMAL)
